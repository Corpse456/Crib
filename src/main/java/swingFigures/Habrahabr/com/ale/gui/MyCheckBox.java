...
public class MyCheckBox extends JCheckBox {
    ...
    static {
        ...
        for (int i = 1; i <= 4; i++) {
            BG_STATES.add(new ImageIcon(MyCheckBox.class.getResource("icons/states/" + i + ".png")));
        }

        // Дополнительное "пустое" состояние выделения
        CHECK_STATES.add(new ImageIcon(new BufferedImage(16, 16, BufferedImage.TYPE_INT_ARGB)));

        // Состояния выделения
        for (int i = 1; i <= 4; i++) {
            CHECK_STATES.add(new ImageIcon(MyCheckBox.class.getResource("icons/states/c" + i + ".png")));
        }
    }
    ...
    private synchronized void updateIcon () {
        ...
        if (iconsCache.containsKey(key)) {
            ...
            BufferedImage b = new BufferedImage(BG_STATES.get(0).getIconWidth(), BG_STATES.get(0).getIconHeight(),
                    BufferedImage.TYPE_INT_ARGB);
            Graphics2D g2d = b.createGraphics();
            g2d.drawImage(BG_STATES.get(bgIcon).getImage(), 0, 0, BG_STATES.get(bgIcon).getImageObserver());
            g2d.drawImage(CHECK_STATES.get(checkIcon).getImage(), 0, 0, CHECK_STATES.get(checkIcon).getImageObserver());
            g2d.dispose();
    ...
    /*
     * Включение/отключение анимации Например при использовании в качестве
     * редактора в ячейке таблицы анимацию стоит отключать
     */
