...
public class ImageResizeComponent extends JComponent {
	...
	this.p2 = new Point(sizerLength + imageIcon.getIconWidth(), sizerLength + imageIcon.getIconHeight());
	...
	if (hitsPoint(p1.x, p1.y, p)) {
	    ...
	    return SW_RESIZE;
	} else if (hitsPoint(p2.x, p2.y, p)) { return SE_RESIZE; }
	return -1;
    }

    private boolean hitsPoint (int x, int y, Point p) {
	return x - sizerLength / 2 <= p.x && p.x <= x + sizerLength / 2 && y - sizerLength / 2 <= p.y
		&& p.y <= y + sizerLength / 2;
    }
	...
	if (imageIcon != null) {
	    ...
	    // Позволяет избежать искажения изображения при различных
	    // непропорциональных ресайзах
	...
	// Типы действий
	none, resizing, dragging
    }

    public Dimension getPreferredSize () {
	// Исходный предпочитаемый размер компонента
	return imageIcon != null
		? new Dimension(imageIcon.getIconWidth() + sizerLength * 2, imageIcon.getIconHeight() + sizerLength * 2)
	...
	f.getContentPane().add(
		new ImageResizeComponent(new ImageIcon(ImageResizeComponent.class.getResource("icons/image.png"))),
		BorderLayout.CENTER);
