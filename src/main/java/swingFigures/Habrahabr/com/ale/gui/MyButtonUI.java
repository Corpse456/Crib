...
public class MyButtonUI extends BasicToggleButtonUI {
    ...
    public MyButtonUI (final AbstractButton button, final boolean changeForeground, float mouseoverTransparency) {
	super();
	...
	this.disabledStaticBottomBg = new Color(Math.min(255, bottomBg.getRed() + 5),
		Math.min(255, bottomBg.getGreen() + 5), Math.min(255, bottomBg.getBlue() + 5));
    }
	...
	if (alwaysDrawBackground) {
	    g2d.setPaint(new GradientPaint(0, 0, c.isEnabled() ? staticTopBg : disabledStaticTopBg, 0, c.getHeight(),
	...
	if (c.isEnabled()) {
		...
		g2d.setPaint(
			new GradientPaint(c.getWidth() / 3, c.getHeight() / 3, topBg, c.getWidth(), c.getHeight(), bg));
		g2d.fill(gp0);
    ...
    public static MyButtonUI setupButtonUI (AbstractButton button, java.util.List<Integer> rounded, Insets insets) {
    ...
    public static MyButtonUI setupDialogButtonUI (AbstractButton button, java.util.List<Integer> rounded) {
	return setupDialogButtonUI(button, rounded, new Insets(4, 12, 4, 12));
    }

    public static MyButtonUI setupDialogButtonUI (AbstractButton button, int rounded, Insets insets) {
